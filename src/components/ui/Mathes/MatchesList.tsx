...
export default function MathesList({
    ...
    //TODO презепускается несколько раз , при выборе (попробовать перенести логику в loadMore)
    useEffect(() => {
        if (
            initRef.current ||
            JSON.stringify(searchParamsSnapRef.current) !== JSON.stringify(searchParams)
        ) {
            console.log("EFFECT 🔁", { initRef, searchParams });
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="fixed top-1/2 left-9 text-3xl">
